...
def tourne(L, taille):
    ...
    for tour in range(0, 8):
        for segment in L:
            L2.append(segmenttourne(segment, tour*2 *
                                    pi/8, [taille[1]/2, taille[0]/2]))
    return L2, taille
...
def crop(L, taille, nouvelletaille, bords):
    ...
    decaleorigine(L, (nouvelletaille[1]-taille[1]
                      >> 1), (nouvelletaille[0]-taille[0] >> 1))
    ...
    Lbords = [Segment([Point(0, nouvelletaille[1]), Point(0, 0)]),
              Segment([Point(0, nouvelletaille[1]), Point(
                  nouvelletaille[0], nouvelletaille[1])]),
    ...
    for segment in L:  # on crée un liste des points d'intersection de chaque segment
        ...
        if len(points) == 0:  # si le segment ne coupe pas un des bords
            ...
            segment.points = [points[0], point_interieur(
                segment.points, nouvelletaille)]
...
def decoupe_tout(L, taille):
    nvLsegment = []
    for segment in L:
        Lpoints = [segment.intersection_avec(
            autre) for autre in L if autre is not segment]
...
def UI():
    ...
    while True:
        if reponse in ("", "Y", "y", "yes", "Yes"):
            ...
            main(nbrsegmentbase=n, taillebase=[taille, taille], taillecrop=[
                 taille2, taille2], bords=bords)
            break
        else:
    ...
    while True:
        if reponse in ("Y", "y", "yes", "Yes"):
            ...
            os.system(
                "cp ./image.svg ./example_images_generees/image{}.svg".format(numero))
            print(
                "Image sauvegardée sous ./example_images_generees/image{}.svg".format(numero))
            break
        elif reponse in ("", "n", "N", "No", "no"):
            break
        else:
